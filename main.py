...
from apify_wallet_fetcher import update_wallet_list
...
if not os.path.exists(CSV_FILE):
    with open(CSV_FILE, 'w', newline='') as file:
        ...
        writer.writerow(["timestamp", "token", "mint", "wallet", "liquidity", "volume", "txns_1h"])
...
# Fetch latest top wallets
update_wallet_list()
with open('tracked_wallets.json', 'r') as f:
    tracked_wallets = json.load(f)

tracked_wallet_addresses = {entry.get('wallet') or entry.get('trackedWalletAddress') for entry in tracked_wallets}
...
def process_token(token):
    ...
    if creator_wallet not in tracked_wallet_addresses:
        return
...
Priority Wallet: {creator_wallet}
...
"""
    ...
    with open(CSV_FILE, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([datetime.utcnow().isoformat(), name, mint, creator_wallet, liquidity, volume, txns_1h])
